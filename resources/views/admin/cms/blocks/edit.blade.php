<form method="POST" action="{{ route("admin.cms.blocks.update", $cmsBlock) }}" class="space-y-4">
    ...
    <div>
        ...
        <textarea name="content" rows="6" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">{{ old("content", $cmsBlock->content) }}</textarea>
    </div>
    <div class="grid grid-cols-2 gap-4">
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Imagem (URL)</label>
            <input type="text" name="image" value="{{ old("image", $cmsBlock->image) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">URL do Vídeo</label>
            <input type="text" name="video_url" value="{{ old("video_url", $cmsBlock->video_url) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
        </div>
    </div>
    <div class="grid grid-cols-2 gap-4">
        <div>
