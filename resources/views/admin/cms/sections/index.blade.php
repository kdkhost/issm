...
@section("title", "Seções - {{ $page->title }}")
@section("page-title", "Seções: {{ $page->title }}")
@push("styles")
<style>
...
<div class="flex justify-between items-center mb-6">
    <div>
        <a href="{{ route("admin.cms.pages.show", $page) }}" class="text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1">
...
<div id="inline-create-form" class="hidden mb-6 bg-white rounded-xl shadow-sm p-6">
    <form method="POST" action="{{ route("admin.cms.sections.store", $page) }}">
        @csrf
...
<div class="bg-white rounded-xl shadow-sm overflow-hidden">
    <ul id="section-list" class="divide-y divide-gray-100">
        ...
        <li data-id="{{ $section->id }}" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
            ...
            <div class="flex items-center gap-2">
                <form method="POST" action="{{ route("admin.cms.sections.toggle", [$page, $section]) }}">
                    @csrf @method("PATCH")
                    <button type="submit" class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors {{ $section->is_active ? "bg-green-600" : "bg-gray-300" }}">
                        <span class="inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform {{ $section->is_active ? "translate-x-[18px]" : "translate-x-1" }}"></span>
                    </button>
                </form>
                <a href="{{ route("admin.cms.blocks.index", [$page, $section]) }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium px-1" data-tooltip="Gerenciar blocos">Blocos</a>
                <form method="POST" action="{{ route("admin.cms.sections.destroy", [$page, $section]) }}" class="inline">
                    @csrf @method("DELETE")
        ...
        @empty
        <li class="px-6 py-10 text-center text-gray-400">Nenhuma seção cadastrada para esta página.</li>
        @endforelse
    </ul>
</div>
<div class="mt-4">
    <a href="{{ route("admin.cms.pages.edit", $page) }}" class="text-gray-600 hover:text-gray-800 text-sm font-medium">← Voltar para edição da página</a>
</div>
@endsection
