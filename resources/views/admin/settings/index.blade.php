...
@section("content")
{{-- ═══════════════════════════════════════════════════════════════
     CSS ENCAPSULADO — não depende de classes do Tailwind que
     podem não estar no build compilado.
═══════════════════════════════════════════════════════════════ --}}
<style>
/* ---------- layout geral ---------- */
...
$groupLabels = [
    ...
    'uploads'     => ['label' => 'Limites de Upload','icon' => 'M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12', 'color' => '#0284c7', 'bg' => '#f0f9ff'],
...
$placeholders = [
    ...
    'contact_notification_to' => 'Ex: admin@issm.org.br. Se vazio, usa o E-mail de Contato.',
    'contact_notification_bcc' => "Ex: membro@empresa.com.br\noutro@empresa.com.br",
...
<div class="cfg-toast" id="cfg-toast">
    ...
    <span class="cfg-toast-text">{{ session('success') }}</span>
...
<form method="POST" action="{{ route('admin.settings.update') }}" enctype="multipart/form-data" id="settings-form">
    ...
    <div class="cfg-wrap">
        ...
        <aside class="cfg-side">
            <nav class="cfg-nav">
                ...
                <ul class="cfg-nav-list" id="cfg-nav">
                    ...
                    @php $m = $groupLabels[$group] ?? ['label'=>ucfirst($group),'icon'=>'','color'=>'#6b7280','bg'=>'#f3f4f6']; @endphp
                    <li>
                        <button type="button"
                            data-group="{{ $group }}"
                            onclick="cfgSwitch('{{ $group }}')"
                            class="cfg-nav-btn {{ $group === $firstGroup ? '--on' : '' }}">
                            <span class="cfg-nav-ico" style="background:{{ $group === $firstGroup ? 'rgba(255,255,255,.2)' : $m['bg'] }}">
                                <svg style="color:{{ $group === $firstGroup ? '#fff' : $m['color'] }}" width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="{{ $m['icon'] }}"/>
                                </svg>
                            </span>
                            <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ $m['label'] }}</span>
                            <span class="cfg-nav-count">{{ $items->count() }}</span>
                        </button>
                    </li>
                    @endforeach
                </ul>
        ...
        <div class="cfg-main">
            ...
            <div id="panel-{{ $group }}" class="cfg-panel {{ $group === $firstGroup ? '--show' : '' }}">

                {{-- Cabeçalho do painel --}}
                <div class="cfg-phead">
                    <div class="cfg-phead-ico" style="background:{{ $m['bg'] }}">
                        <svg width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.75" style="color:{{ $m['color'] }}">
                            <path stroke-linecap="round" stroke-linejoin="round" d="{{ $m['icon'] }}"/>
                        </svg>
                    </div>
                    <div>
                ...
                <div class="cfg-card cfg-card-instructions" style="border-left:4px solid #ea580c">
                    ...
                    <div class="cfg-card-body" style="font-size:.8125rem;line-height:1.6">
                        <ol>
                            ...
                            <li><strong>Crie uma Conta de Servico:</strong> em Credenciais, clique no botao azul <strong>+ CRIAR CREDENCIAIS</strong> (topo direito), escolha <strong>Conta de servico</strong>, de o nome <code>portal-transparencia</code>, clique <strong>Criar e continuar</strong> e depois <strong>Concluir</strong>.</li>
                            <li><strong>Gere a chave JSON:</strong> na mesma pagina de Credenciais, procure a secao <strong>Contas de servico</strong>, clique no nome da conta criada, va na aba <strong>CHAVES</strong>, clique <strong>ADICIONAR CHAVE > Criar nova chave</strong>, escolha <strong>JSON</strong> e clique <strong>CRIAR</strong>. O arquivo baixara automaticamente.</li>
                ...
                ════════════════════════════════════════ --}}
                @if($group === 'partners')
                @php $cv = fn(string $k, $d='') => $items->firstWhere('key',$k)?->value ?? $d; @endphp
                ...
                <div class="cfg-preview-bar">
                    ...
                    <span>Velocidade: <strong id="lbl-speed">{{ $cv('partners_carousel_speed','3000') }}</strong> ms</span>
                    <span>Logos/tela: <strong id="lbl-perview">{{ $cv('partners_carousel_per_view','4') }}</strong></span>
                    <span>Altura: <strong id="lbl-height">{{ $cv('partners_logo_height','64') }}</strong> px</span>
                    <strong id="lbl-effect" style="color:#0d9488;text-transform:capitalize">{{ $cv('partners_carousel_effect','slide') }}</strong>
                </div>
                        ...
                        <div class="cfg-grid">
                            ...
                            <div>
                                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.75rem">
                                    ...
                                    <span class="cfg-badge --green" id="badge-speed">{{ $cv('partners_carousel_speed','3000') }} ms</span>
                                </div>
                                <input type="range" name="partners_carousel_speed" min="500" max="10000" step="500"
                                       value="{{ $cv('partners_carousel_speed','3000') }}" class="cfg-range"
                            ...
                            <div>
                                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.75rem">
                                    ...
                                    <span class="cfg-badge --blue" id="badge-perview">{{ $cv('partners_carousel_per_view','4') }}</span>
                                </div>
                                <input type="range" name="partners_carousel_per_view" min="1" max="8" step="1"
                                       value="{{ $cv('partners_carousel_per_view','4') }}" class="cfg-range --blue"
                                ...
                                <div class="cfg-range-ends">@for($i=1;$i<=8;$i++)<span>{{ $i }}</span>@endfor</div>
                            </div>
                            {{-- Altura --}}
                            <div>
                                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.75rem">
                                    <label class="cfg-label" style="margin:0">Altura dos logos</label>
                                    <span class="cfg-badge --purple" id="badge-height">{{ $cv('partners_logo_height','64') }} px</span>
                                </div>
                                <input type="range" name="partners_logo_height" min="32" max="140" step="4"
                                       value="{{ $cv('partners_logo_height','64') }}" class="cfg-range --purple"
                            ...
                            <div>
                                ...
                                @php $curEffect = $cv('partners_carousel_effect','slide'); @endphp
                                <div class="cfg-radio-grid">
                                    @foreach(['slide'=>['Slide','M9 5l7 7-7 7'],'fade'=>['Fade','M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4'],'coverflow'=>['Coverflow','M4 6h16M4 12h16M4 18h7']] as $val=>[$lbl,$ico])
                                    <label class="cfg-radio-card">
                                        <input type="radio" name="partners_carousel_effect" value="{{ $val }}" {{ $curEffect===$val?'checked':'' }}
                                               onchange="document.getElementById('lbl-effect').textContent='{{ $lbl }}'">
                                        <div class="cfg-radio-inner">
                                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $ico }}"/></svg>
                                            <span>{{ $lbl }}</span>
                                        </div>
                ...
                {{-- Toggles do carrossel --}}
                @php $toggles=['partners_carousel_autoplay'=>'Autoplay automático','partners_carousel_loop'=>'Loop infinito','partners_carousel_dots'=>'Mostrar indicadores','partners_carousel_arrows'=>'Mostrar setas']; @endphp
                <div class="cfg-card">
                    <div class="cfg-card-head">
                    ...
                    <div class="cfg-card-body">
                        <div class="cfg-toggles-grid" style="grid-template-columns:repeat(2,1fr)">
                            ...
                            @php $tVal=$items->firstWhere('key',$tKey)?->value??'0'; @endphp
                            <input type="hidden" name="{{ $tKey }}" value="0">
                            <label class="cfg-toggle">
                                <span class="cfg-toggle-label">{{ $tLabel }}</span>
                                <span class="cfg-switch">
                                    <input type="checkbox" name="{{ $tKey }}" value="1" {{ $tVal=='1'?'checked':'' }}>
                        ...
                        <div class="cfg-grid">
                            ...
                            <div class="{{ $wide ? 'cfg-wide' : '' }}">
                                ...
                                <div>
                                    @if($s->value)
                                    <div class="cfg-img-current">
                                        <img src="{{ asset('media/'.$s->value) }}" alt="{{ $fLabel }}">
                                        <div class="cfg-img-info">
                                            <div class="cfg-img-name">{{ basename($s->value) }}</div>
                                            <div class="cfg-img-hint">Imagem atual</div>
                                            <label class="cfg-img-remove">
                                                <input type="checkbox" name="{{ $s->key }}_remove" value="1"> Remover
                                            </label>
                                        </div>
                                    </div>
                                    @endif
                                    <label class="cfg-upload-zone">
                                        ...
                                        <span class="cfg-upload-text" id="lbl-{{ $s->key }}">{{ $s->value ? 'Clique para substituir' : 'Selecionar arquivo' }}</span>
                                        <span class="cfg-upload-hint">PNG · JPG · SVG · WEBP</span>
                                        <input type="file" name="{{ $s->key }}_file" accept="image/*" data-no-dropzone="1" style="display:none"
                                               onchange="document.getElementById('lbl-{{ $s->key }}').textContent=this.files[0]?.name??'Selecionado'">
                                    </label>
                                    <input type="hidden" name="{{ $s->key }}" value="{{ $s->value }}">
                                </div>

                                {{-- NUMBER (partner settings fallback) --}}
                                @elseif($s->type==='number' || in_array($s->key,['partners_carousel_speed','partners_logo_height','partners_carousel_per_view']))
                                <div class="cfg-num-wrap">
                                    <input type="number" name="{{ $s->key }}" value="{{ $s->value }}" class="cfg-num-input">
                                    <span class="cfg-num-unit">@if(str_contains($s->key,'_speed'))ms @elseif(str_contains($s->key,'_height'))px @elseif(str_contains($s->key,'_mb'))MB @elseif(str_contains($s->key,'opacity'))% @else un @endif</span>
                                </div>
                                ...
                                <div class="cfg-input-wrap">
                                    ...
                                    <input type="password" name="{{ $s->key }}" value="{{ $s->value }}"
                                ...
                                <div class="cfg-input-wrap">
                                    ...
                                    <input type="text" name="{{ $s->key }}" value="{{ $s->value }}"
                                           class="cfg-input {{ $hasIcon ? 'cfg-input--iconed' : '' }}"
                                           placeholder="{{ $ph }}">
                                </div>
                                @endif
                            </div>
                ...
                <div class="cfg-card">
                    ...
                    <div class="cfg-card-body">
                        <div class="cfg-toggles-grid">
                            @foreach($booleans as $s)
                            @php $bLabel=$s->label??str_replace('_',' ',ucwords($s->key,'_')); @endphp
                            <input type="hidden" name="{{ $s->key }}" value="0">
                            <label class="cfg-toggle">
                                <span class="cfg-toggle-label">{{ $bLabel }}</span>
                                <span class="cfg-switch">
