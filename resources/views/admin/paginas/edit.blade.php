...
@section("content")
<div class="mb-4">
    <a href="{{ route('admin.paginas.index') }}" class="text-sm text-green-700 hover:text-green-900">&larr; Voltar</a>
</div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-5 gap-6">
        ...
        <div class="xl:col-span-3 space-y-0">
            ...
            <div class="bg-white rounded-xl shadow-sm mb-6 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
                    <h3 class="font-bold text-gray-800">Conteudo da Pagina</h3>
                </div>
                <div class="p-6 space-y-5">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Titulo *</label>
                        <input id="ptitle" type="text" name="title" value="{{ old('title', $page->title) }}" required placeholder="Titulo da pagina">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Conteudo *</label>
                        <textarea name="content" rows="10" required class="wysiwyg">{{ old('content', $page->content) }}</textarea>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm mb-6 overflow-hidden">
                ...
                <div class="p-6 space-y-5">
                    <div>
                        ...
                        <input id="mt" type="text" name="meta_title" value="{{ old('meta_title', $page->meta_title) }}" placeholder="Titulo para SEO (max. 60 caracteres)">
                    </div>
                    <div>
                        ...
                        <textarea id="md" name="meta_description" rows="3" placeholder="Descricao para SEO (max. 160 caracteres)">{{ old('meta_description', $page->meta_description) }}</textarea>
                    </div>
                    <div>
                        ...
                        <input id="st" name="seo_tags" type="text" value="{{ old('seo_tags', $page->seo_tags) }}" placeholder="#issm, #meioambiente, #sustentabilidade, #ods2030, #floresta, #preservacao">
            ...
            <div class="bg-white rounded-xl shadow-sm mb-6 overflow-hidden">
                ...
                <div class="p-6 space-y-5">
                    <div>
                        ...
                        <input id="ot" type="text" name="og_title" value="{{ old('og_title', $page->og_title) }}" placeholder="Ex: og titulo">
                    </div>
                    <div>
                        ...
                        <textarea id="od" name="og_description" rows="2" placeholder="Ex: og descricao">{{ old('og_description', $page->og_description) }}</textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">OG Image</label>
                        <input id="oi" type="text" name="og_image" value="{{ old('og_image', $page->og_image) }}" placeholder="media/banners/imagem.jpg">
            ...
            <div class="bg-white rounded-xl shadow-sm mb-6 overflow-hidden">
                ...
                <div class="p-6 space-y-5">
                    <div>
                        ...
                        <input id="cu" type="url" name="canonical_url" value="{{ old('canonical_url', $page->canonical_url ?? route('pages.show', $page->slug)) }}">
                    </div>
                    <div>
            ...
            <div class="flex justify-end gap-3 mb-6">
                <a href="{{ route('admin.paginas.index') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 text-sm">Cancelar</a>
                <button type="submit" class="bg-green-700 text-white px-6 py-2 rounded-lg hover:bg-green-800 font-medium text-sm">Atualizar</button>
            </div>
        </div>
        ...
        <div class="xl:col-span-2 space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm p-4">
                ...
                <div class="font-sans">
                    <div id="pgt" class="text-[#1a0dab] text-base leading-tight truncate hover:underline cursor-pointer">{{ $page->meta_title ?: $page->title ?: 'Titulo' }}</div>
                    <div id="pgu" class="text-xs text-[#006621] mt-0.5">{{ url('/') }}/pagina/{{ $page->slug }}</div>
                    <p id="pgd" class="text-sm text-[#4d5156] leading-snug mt-1 line-clamp-2">{{ $page->meta_description ?: 'Descricao...' }}</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-4">
                ...
                <div class="border border-gray-200 rounded-lg overflow-hidden">
                    <div id="pfi" class="bg-gray-200 h-28 bg-cover bg-center" style="background-image: url('{{ $page->og_image ? asset('media/' . $page->og_image) : asset('media/' . \App\Models\Setting::get('og_image', 'logo.png')) }}')"></div>
                    <div class="p-3 bg-white">
                        <p id="pfu" class="text-[10px] text-gray-500 uppercase truncate">{{ strtoupper(parse_url(url('/'), PHP_URL_HOST)) }}</p>
                        <h5 id="pft" class="text-sm font-bold text-gray-900 truncate mt-0.5">{{ $page->og_title ?: $page->meta_title ?: $page->title ?: 'Titulo' }}</h5>
                        <p id="pfd" class="text-xs text-gray-500 leading-snug mt-0.5 line-clamp-2">{{ $page->og_description ?: $page->meta_description ?: 'Descricao...' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-4">
                ...
                <div class="flex gap-3 bg-[#e1ffc7] p-3 rounded-lg rounded-tl-none">
                    <div id="pwi" class="w-12 h-12 rounded bg-gray-300 bg-cover bg-center shrink-0" style="background-image: url('{{ $page->og_image ? asset('media/' . $page->og_image) : asset('media/' . \App\Models\Setting::get('og_image', 'logo.png')) }}')"></div>
                    <div class="min-w-0">
                        <p id="pwt" class="text-sm font-medium text-gray-900 truncate">{{ $page->og_title ?: $page->meta_title ?: $page->title ?: 'Titulo' }}</p>
                        <p id="pwd" class="text-xs text-gray-600 line-clamp-2">{{ $page->og_description ?: $page->meta_description ?: 'Descricao...' }}</p>
...
(function() {
    ...
    function score() {
        ...
        el.tips.innerHTML = t.length === 0
            ? '<p class="text-green-600 flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>SEO otimo!</p>'
            : t.map(x => '<p class="text-gray-600 flex items-start gap-1"><svg class="w-3 h-3 mt-0.5 text-amber-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg><span>' + x + '</span></p>').join('');
    }
