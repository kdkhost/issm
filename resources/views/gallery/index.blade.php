@extends("layouts.app")
@section("title", $cmsPage?->meta_title ?? $cmsPage?->title ?? "Galeria - ISSM")
    ...
    <div style="max-width:1280px;margin:0 auto;padding:0 16px;">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:10px;">
            <div class="gal-stat">
                ...
                {{ $totalGallery }} foto{{ $totalGallery != 1 ? 's' : '' }} da galeria
            </div>
            <div class="gal-stat">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                {{ $totalProjects }} imagen{{ $totalProjects != 1 ? 's' : '' }} de projetos
            </div>
        </div>
    </div>
</div>
    ...
    <div style="max-width:1280px;margin:0 auto;padding:0 16px;">
        <div class="gal-filters">
            ...
            @foreach($filterOptions as $opt)
                ...
                @endphp
                <a href="{{ $href }}" class="gal-chip {{ $isActive ? '--active' : '' }}">
                    {{ $opt }}
                    @if(isset($filterCounts[$opt]))
                    <span class="gal-chip-count">{{ $filterCounts[$opt] }}</span>
                    @endif
                </a>
            @endforeach
        </div>
...
<section style="padding:40px 0 60px;background:#f9fafb;min-height:60vh;">
    <div style="max-width:1280px;margin:0 auto;padding:0 16px;">
        ...
        <div class="gal-grid" id="gallery-grid">
            @foreach($allItems->values() as $i => $item)
            <div class="gal-card"
                 data-index="{{ $i }}"
                 data-src="{{ asset('media/' . $item->image) }}"
                 data-caption="{{ $item->title }}{{ $item->album ? ' — '.$item->album : '' }}">
                <img src="{{ asset('media/' . $item->image) }}" alt="{{ $item->title }}" loading="lazy">
                <div class="gal-overlay">
                    <span class="gal-overlay-badge {{ $item->type === 'project' ? '--project' : '--gallery' }}">
                        {{ $item->type === 'project' ? 'Projeto' : 'Galeria' }}
                    </span>
                    @if($item->link)
                    <a href="{{ $item->link }}" class="gal-overlay-link" title="Ver projeto" onclick="event.stopPropagation();">
                    ...
                    @endif
                    <div>
                        <p class="gal-overlay-title">{{ $item->title }}</p>
                        @if($item->album)
                        <span class="gal-overlay-album">{{ $item->album }}</span>
                        @endif
                    </div>
                </div>
            </div>
