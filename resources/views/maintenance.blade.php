...
<head>
    ...
    <title>{{ $title ?? 'Site em Manutenção' }}</title>
    @php
    ...
    @endphp
    @if($siteFavicon)
    <link rel="icon" href="{{ asset('media/' . $siteFavicon) }}">
    @endif
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .mnt-bg {
            ...
            background: {{ $hasBg ? '#000' : $bgCol }};
        }
        @if($hasBg)
        .mnt-bg::before {
            content: '';
            position: absolute; inset: 0;
            background: url('{{ asset("media/" . $bgImage) }}') center/cover no-repeat;
            opacity: .45;
        }
        .mnt-bg::after {
            content: '';
    ...
    <div class="mnt-wrap">
        <div class="mnt-card">
            ...
            <div class="mnt-logo-ring">
                @if($displayLogo)
                    <img src="{{ asset('media/' . $displayLogo) }}" alt="{{ $siteName }}">
                @else
                    <span>{{ mb_strtoupper(mb_substr($siteName, 0, 4)) }}</span>
                @endif
            </div>
            ...
            {{-- Título --}}
            <h1 class="mnt-title">{{ $title ?? 'Site em Manutenção' }}</h1>

            {{-- Mensagem --}}
            <div class="mnt-msg">{!! $message ?? 'Estamos realizando melhorias no nosso site. Em breve estaremos de volta!' !!}</div>
            ...
            {{-- Contagem regressiva --}}
            @if(!empty($showCountdown) && $showCountdown && !empty($returnDate))
            <div class="mnt-countdown" id="countdown" data-target="{{ $returnDate }}">
            ...
            {{-- Redes sociais --}}
            @if(!empty($showSocial) && $showSocial)
            @php
            ...
            <div class="mnt-social">
                ...
                <a href="https://wa.me/55{{ $wa }}" target="_blank" rel="noopener" title="WhatsApp">
            ...
            {{-- Acesso admin --}}
            <a href="{{ route('login') }}" class="mnt-admin">Acesso Administrativo</a>
